...
final_decision_tree <- finalize_workflow(tree_wf,
                                       best_tree
)

print('*********** DECISION TREE **************')
tree_test <- last_fit(final_decision_tree,income_split) %>%
  collect_metrics()
...
nnet_test <- last_fit(final_decision_nnet,income_split) %>%
  collect_metrics()
...
print('*********** KNN **************')
knn_test <- last_fit(final_decision_knn,income_split) %>%
  collect_metrics()
...
# what is the best value of lambda?
top_svm  <- show_best(tree_svm, metric = "accuracy")
best_svm <- select_best(tree_svm, metric = "accuracy")
final_decision_svm <- finalize_workflow(tree_svm,
                                        best_svm
)

print('*********** SUPPORT VECTOR MACHINE **************')
svm_test <- last_fit(final_decision_svm,income_split) %>%
  collect_metrics()
